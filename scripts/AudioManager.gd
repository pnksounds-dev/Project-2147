...
func _update_bus_volumes() -> void:
	# Set all buses to 100% volume
	AudioServer.set_bus_volume_db(Bus.MASTER, linear_to_db(1.0))  # Master at 100%
...
func _on_music_finished() -> void:
	...
	if _music_player.stream and _music_player.stream.loop:
		_music_player.play()
